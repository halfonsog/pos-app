...
│  State, API, Utils, Toast, Sidebar, + 14 módulos)   │
└──────────────────────┬──────────────────────────────┘
...
┌──────────────────────▼──────────────────────────────┐
│ SQLite: database/database.db (28 tablas)            │
└─────────────────────────────────────────────────────┘
```
...
| Tests | Jest (instalado, sin tests escritos) | — |
...
│   │   ├── routes/            # api.js (monta todo) + 18 routers por dominio
│   │   ├── controllers/       # 18 controladores (lógica de negocio + SQL)
...
│       │   └── modules/       # 17 módulos (uno por pantalla/dominio, sin modales: vistas completas)
│       ├── lib/               # Librerías vendor (offline) — NO tocar
│       └── uploads/productos/ # Fotos subidas
├── tests/                     # Jest + supertest (BD temporal desde migraciones)
│   ├── helpers/testDb.js
│   └── *.test.js              # 12 suites, 160 tests (seguridad, usuarios, productos, inventario,
│                              # préstamos, vector-fiscal, banco, mayoristas, encargos, servicios, contabilidad)
...
> 🧹 Limpieza 2026-08-05 (aprobada por el propietario): movidos a `deleted/` los esquemas obsoletos (`schema.sql`, `database.schema.sql`), `fix-migration.js`, `dashboard.html`, `vendor.html`, `views/`, `confirm-modal.js` y `download-libs.js`. Eliminado `pos.db` (0 bytes). **Regla permanente: mantener este árbol actualizado con cada cambio (aplica a todos los docs).**
...
3. `app.use('/api', apiRoutes)` → `routes/api.js` monta los 18 routers.
...
| `npm run db:seed` | `database/scripts/seeds.js` | ✅ OK (corregido en Sprint 0) |
| `npm run db:reset` | `init.js && seeds.js` | ✅ OK (corregido en Sprint 0) |
| `npm test` | `jest` | ✅ OK — 59 tests (matriz de acceso) contra BD temporal |

> 🗑️ Eliminado el script `download-libs` (2026-08-05, aprobado por el propietario): las librerías vendor ya están en `src/frontend/lib/`.
...
2. **SPA sin framework**: cada módulo es un objeto global; cada "vista" es un método que genera un template string gigante y lo inyecta en `$('#app')`. Re-render completo en cada navegación.
3. **Autenticación JWT stateless**: `logout` es un no-op; el token vive 24h en `localStorage`.
4. **Autorización (RBAC) en backend** ✅ (Sprint 0): `requireRole` en `middleware/auth.js` + matriz rol↔endpoint (ver 03-api.md y tests/seguridad.test.js).
5. **Offline-first parcial**: las librerías son locales, pero la app no funciona sin el backend (SW sin caché).
6. **Distribución objetivo (D17)**: app nativa por plataforma/SO con **todas las dependencias incluidas** en el instalador. El navegador/PWA actual es solo el vehículo temporal para terminar el desarrollo y ponerla a punto.
