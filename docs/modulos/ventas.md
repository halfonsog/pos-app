...
- Resumen de turno: el card **Rentabilidad fue sustituido por el card unificado "Desglose por Prioridades"** (`Ventas.renderDesgloseTurno`): recaudado → impuestos → costos base → gastos fijos → préstamos → inversiones → **servicios del turno (línea informativa, D35)** → margen → ganancias máx → **excedente con flecha al destino** (inversiones → préstamos → ganancias). Gastos en rojo, margen rojo/verde según signo, excedente verde. Los gastos fijos y financieros son **equivalentes** al período (Σ configuracion_gastos + Σ salarios, y próximos vencimientos de préstamos/inversiones, prorrateados según venta neta ÷ ventas proyectadas); la verificación `comparacion_gastos.real_pct` = (fijos + financieros) ÷ venta neta debe igualar al proyectado. ⚠ **Solo informativo** (no persiste). El turno usa el **mundo real** (todas las ventas); el cierre de mes usa solo las gravables (D30). En el cierre de mes el excedente deberá aplicar pago a vencimientos (reducir plazo) — pendiente.
